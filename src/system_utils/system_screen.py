"""
NOTE: this code was originally generated by chatGPT, and is still mostly that (very little edits were made).
...
# object to store the screen's properties and handle points/pixel conversions.
class Screen:
    dpi: float
    size: Tuple[int, int]
    scaling: float

    design_size: Tuple[int, int] = (2560, 1440) # size of the screen that the hard-coded pixel values were calculated for.
    layout_scale: float = 1.0

    @classmethod
    def set_metrics(cls, dpi: float, size: Tuple[int, int], scaling: float) -> None:
        cls.dpi = dpi
        cls.size = size
        cls.scaling = scaling
        cls.layout_scale = min(
            size[0] / cls.design_size[0],
            size[1] / cls.design_size[1],
        )

    @classmethod
    def px(cls, value: float) -> int:
        return max(1, round(value * cls.layout_scale))
...
def getScreenInfo(root) -> Tuple[float, Tuple[int, int], float]:
    ...
    # Last resort: use Tk's logical points
    return float(root.winfo_fpixels("1i")), root.winfo_screenwidth(), root.winfo_screenheight()
...
def _getLinuxScreenInfo(root) -> Optional[Tuple[float, Tuple[int, int], float]]:
    ...
    def _getWaylandInfoViaGdk(GdkModule, _root) -> Optional[Tuple[float, Tuple[int, int], float]]:
        """
        ...
        try:
            ...
            try:
                pixelW = int(_root.winfo_screenwidth() * scale)
                pixelH = int(_root.winfo_screenheight() * scale)
            except (AttributeError, ValueError):
                return None
