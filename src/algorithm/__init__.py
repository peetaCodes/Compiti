...
from src.storage.datatypes import UID, ScheduleDay, RawSequence, ProcessedSequence
...
class InputTransformer:
    ...
    def generate_schedule_inputs_from_tasks(
        ...
        Args:
          ...
          sequencesNames: mapping from original (generated) sequence names to new (customised) names.
        ...
        for uid, task in tasksStore.items():
            due_lookup.setdefault(task.dueDate, []).append((uid, task))
        ...
        for seq in sequences:
            ...
            seq_days_list: List[ScheduleDay] = []
            for d in seq:
                day: ScheduleDay = []
